...
export default function CommunityDetailAdmin() {
  ...
  /* ดึงข้อมูลชุมชน (ของ Admin เอง) */
  useEffect(() => {
    (async () => {
      try {
        const response = await getCommunityDetailByAdmin();
    ...
    <div className="w-full space-y-4">
      ...
      <div>
        <Breadcrumb
          current={{
            ...
            to: "/admin/community/own", // path ของหน้าปัจจุบัน
            fromSidebar: true,
          }}
        />
      </div>
      ...
      <div className="rounded-3xl border border-slate-200 bg-white shadow-sm box-border w-full p-6">
        ...
        <div className="px-6 pt-5 pb-3 flex items-center justify-between">
          ...
          >
            {/* สำหรับ Admin หน้า own อาจไม่มีรายการให้ย้อนกลับ เลยไม่แสดงลูกศร */}
                  ...
                  {storeLists.length ? (
                    <div className="space-y-4">
                      {storeLists.map((store: any) => (
                        <div
                          key={store.id}
                          onClick={() => navigate(`/admin/community/store/${store.id}`)}
                          className="cursor-pointer"
                        >
                          <ItemCard
                            image={resolveBackendUploadUrl(
                  ...
                  {homestayLists.length ? (
                    <div className="space-y-4">
                      {homestayLists.map((homestay: any) => (
                        <div
                          key={homestay.id}
                          onClick={() => navigate(`/admin/community/homestay/${homestay.id}`)}
                          className="cursor-pointer"
                        >
                          <ItemCard
                            image={resolveBackendUploadUrl(
