// src/Pages/Member/EditPackagePage.tsx
/**
 * - หน้าแก้ไขแพ็กเกจ (บทบาท Member)
 * - ดึงรายละเอียดแพ็กเกจด้วย /member/package/:id แล้วเติมฟอร์ม
 * - ใช้ Zod validate แบบเดียวกับหน้า Create
 * - ส่งอัปเดตด้วย PUT /member/package/:id
 */
...
export const EditPackagePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setLoading(true);
        const res = await axios.get(`${apiUrl}/member/package/${id}`, {
          withCredentials: true,
        });
        ...
        // ตั้งแท็กเริ่มต้น
        setSelectedTags((tags as any[]).map((t) => ({ id: Number(t.id), name: t.name ?? "" })));
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await axios.put(`${apiUrl}/member/package/${id}`, payload, {
        withCredentials: true,
      ...
      console.error("Edit package (member) error:", error?.response?.data);
      setErrorMessage(
