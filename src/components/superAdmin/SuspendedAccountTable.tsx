// ==================================================
// SuspendedAccountTable.tsx
// Super Admin: ตาราง "จัดการบัญชีที่ถูกระงับ" (UI เท่านั้น / ยังไม่ผูก DB)
// เงื่อนไข:
...
export type SuspendedAccountRow = {
  ...
  communityName: string;  // ชุมชน
  email: string;          // อีเมล
};
// 2) รองรับส่งข้อมูลจากภายนอก (ถ้ามี) หรือให้ component fetch เอง
type Props = {
  rowsFromApi?: SuspendedAccountRow[];
};

export default function SuspendedAccountTable({ rowsFromApi }: Props) {
  // 3) state เก็บข้อมูลจาก DB/API
  const [data, setData] = useState<SuspendedAccountRow[]>([]);
